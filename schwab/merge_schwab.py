    ...
    for i, transaction in enumerate(transactions):
        if transaction.get("Action") == "Cancel Sell":
            ...
            for j in range(i-1, -1, -1):
                if (transactions[j]["Amount"] == transaction["Amount"][1:] and 
                   transactions[j]["Quantity"] == transaction["Quantity"]):
...
def update_transaction_price_for_vesting(transaction, tax_transactions_activity, fair_value_price_dict):
    month_year_str = date_parse(transaction.get("Date")).strftime(MONTH_YEAR_FORMAT)
    number_of_shares_sold = transaction.get("Quantity")
    number_of_shares_sold_for_taxes_list = tax_transactions_activity.get(month_year_str, [])
...
def update_transactions(transactions, fair_value_price_dict, tax_transactions_dict):
    ...
    for i, transaction in enumerate(transactions):
        ...
        if transaction.get("Action") == "Stock Plan Activity":
            should_remove_transaction = update_transaction_price_for_vesting(transaction, tax_transactions_activity, fair_value_price_dict)
            if should_remove_transaction:
                indexes_to_remove.append(i)
        elif transaction.get("Action") == "Sell":
            should_remove_transaction = remove_shares_sold_for_taxes(transaction, tax_transactions_sell)
            if should_remove_transaction:
                indexes_to_remove.append(i)
    remove_indexes_from_list(transactions, indexes_to_remove)    
    ...
    with open(equity_transactions_path, "r") as f:
        ...
        try:
            while True:
                ...
                fair_market_price = float(rsu_information.get("FairMarketValuePrice").replace("$", ""))
                number_of_shares_sold_for_taxes = rsu_information.get("SharesSoldWithheldForTaxes")
                data = EquityData(date, fair_market_price, number_of_shares_sold_for_taxes)
...
def get_vesting_price_dict(equity_data):
    ...
    for equity_item in equity_data:
        ...
        if fair_value_price_dict.get(month_year_str, equity_item.vesting_price) != equity_item.vesting_price:
            print(f"Unexpected price mismatch for month: {month_year_str}. "
