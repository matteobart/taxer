...
from tax_methods import fifo_comparator, lifo_comparator, high_cost_comparator, low_cost_comparator, tax_optimizer_comparator
...
class TestHighCost(unittest.TestCase):
    ...
    def test_highcost1_1(self):
        sell_price = None
        sell_datetime = None
        tax_method_cmp = lambda x, y: high_cost_comparator(x, y, sell_price, sell_datetime)
        transactions = transactions1.copy()
        sorted_transactions = sorted(transactions, key=cmp_to_key(tax_method_cmp))
        expected_transactions = [
            Transaction(30, 10, datetime(2024, 4, 1)),
...
class TestLowCost(unittest.TestCase):
    ...
    def test_lowcost1_1(self):
        sell_price = None
        sell_datetime = None
        tax_method_cmp = lambda x, y: low_cost_comparator(x, y, sell_price, sell_datetime)
        transactions = transactions1.copy()
        sorted_transactions = sorted(transactions, key=cmp_to_key(tax_method_cmp))
        expected_transactions = [
            Transaction(20, 2, datetime(2024, 3, 1)),
...
class TestTaxOptimizer(unittest.TestCase):
    ...
    def test_taxoptimizer_singles_1(self):
        ...
        comparison_result = tax_optimizer_comparator(tenPrice, fivePrice, 5, datetime(2024, 4, 2))
        self.assertEqual(comparison_result, -1)

        comparison_result_flipped = tax_optimizer_comparator(fivePrice, tenPrice, 5, datetime(2024, 4, 2))
        self.assertEqual(comparison_result_flipped, 1)

    def test_taxoptimizer1_1(self):
        sell_price = 5
        sell_datetime = datetime(2025, 3, 10)
        tax_method_cmp = lambda x, y: tax_optimizer_comparator(x, y, sell_price, sell_datetime)
        transactions = transactions1.copy()
        sorted_transactions = sorted(transactions, key=cmp_to_key(tax_method_cmp))
        expected_transactions = [
            Transaction(30, 10, datetime(2024, 4, 1)),
